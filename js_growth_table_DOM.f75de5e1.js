...
const getCount = (direction)=>{
    return direction === 'row' ? tBody.querySelectorAll('tr').length : tBody.querySelectorAll('tr:first-child td').length;
};
const addRow = (e)=>{
    const newRow = tBody.insertRow();
    const cells = tBody.querySelectorAll('tbody tr:first-child td');
    for(let i = 0; i < cells.length; i++)newRow.insertCell();
...
//# sourceMappingURL=js_growth_table_DOM.f75de5e1.js.map
